...
/////////////////////////////////////////////////////////////////////////
	...
	test.before(async function () {
		logger.info("Version 2.7.0 - Wizard2.0 ");
...
//unskip
	test.it.skip('Manage page: correct list of whitelisted addresses is displayed for tier#1 after addition',
		async function () {
			...
			for (let i = 0; i < addresses.length; i++) console.log("address[" + i + "]= " + addresses[i]);

			console.log(Investor2.account)
			console.log(e2eWhitelist.tiers[tierNumber - 1].whitelist[0].address);
			console.log(e2eWhitelist.tiers[tierNumber - 1].whitelist[1].address);
	...
	test.it.skip('Manage page: correct list of whitelisted addresses is displayed for tier#1 after addition',
		async function () {
			...
			let addresses = await mngPage.getWhitelistAddresses(tierNumber);
			for (let i = 0; i < addresses.length; i++) console.log("address[" + i + "]= " + addresses[i]);
			console.log(ReservedAddress.account)
			console.log(Investor2.account)
			console.log(e2eWhitelist.tiers[tierNumber - 1].whitelist[0].address);
			console.log(e2eWhitelist.tiers[tierNumber - 1].whitelist[1].address);
	...
	test.it('Tier#1 has finished as scheduled',
		async function () {
			...
			while ((await investPage.isCurrentTier1()) && (counter-- > 0));
	...
	test.it('Tier #2 started immideatelly after tier#1 is finished ',
		async function () {
			...
			while (!(await investPage.isCurrentTier2()) && (counter-- > 0));
			let result = (counter > 0);
	...
	test.it('Manage page: correct list of whitelisted addresses is displayed for tier#2 after addition',
		async function () {
			...
			let addresses = await mngPage.getWhitelistAddresses(tierNumber);
			let result = (addresses.length === 3)
				&& addresses.includes(Investor3.account)
				&& addresses.includes(e2eWhitelist.tiers[tierNumber - 1].whitelist[0].address)
...
//////////////////////// Test SUITE #2 /////////////////////////////
	...
	test.it('Tier#1 is finished as scheduled',
		async function () {
			let investor = Investor1;
...
//////// TIER#2 ////////////
	test.it('Manage page,tier #2: field minCap disabled if tier has started',
		async function () {
			let owner = Owner;
			let tierNumber = 2;
			assert.equal(await owner.setMetaMaskAccount(), true, "Can not set Metamask account");
	...
	test.it('Investor is able to buy maxCap',
		async function () {
			...
			investor.tokenBalance = e2eMinCap.tiers[1].supply;
			await investor.contribute(contribution);
			let balance = await investor.getBalanceFromInvestPage(e2eMinCap);
			let result = (Math.abs(parseFloat(balance) - parseFloat(investor.tokenBalance)) < 0.1);
			return await assert.equal(result, true, "Test FAILED.Investor can not  buy  maxCap");
	...
	test.it('Tier#2 is finished as scheduled',
		async function () {
			let investor = Investor1;
			assert.equal(await investor.openInvestPage(e2eMinCap), true, 'Investor can not open Invest page');
			let counter = 50;
			...
			while ((await investPage.isCurrentTier3()) && (counter-- > 0));
			let result = (counter > 0);
			return await assert.equal(result, true, "Test FAILED. Tier has not finished in time");
		});
...
//////// TIER#3 ///////////
	...
	test.it('Tier#3 is finished as scheduled',
		async function () {
			let investor = Investor3;
			assert.equal(await investor.openInvestPage(e2eMinCap), true, 'Investor can not open Invest page');
			let counter = 240;
	...
	test.it('Reserved address#1 has received correct percent of tokens after finalization',
		async function () {
			...
			let shouldBe = e2eMinCap.reservedTokens[0].value *
				( Investor1.tokenBalance + Investor2.tokenBalance + Investor3.tokenBalance) / 100;
			logger.info("Investor should receive  = " + shouldBe);
			logger.info("Investor has received balance = " + balance);
			return await assert.equal(shouldBe, balance, "Test FAILED.'Investor has received " + balance + " tokens instead " + shouldBe);
		});
