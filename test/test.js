...
//////////////////////////////////////////////////////////////////////////////
    test.it.skip('SelfTest', async function() {
   ...
   //investor1.setMetaMaskAccount();
    ...
    test.it('Investor can NOT contribute from another network', async function() {
        investor=user4_40cA;
        await investor.setMetaMaskAccount();
        ...
        assert.equal(b, false, "Test FAILED. Investor can contribute from another network");
        logger.error("Test PASSED.Investor can NOT contribute from another network. ");

    });
    test.it('Investor can NOT contribute less than minCap in first transaction', async function() {
    ...
    test.it('Investor can NOT contribute more than supply in tier', async function() {
        investor=user77_27F2;
        await investor.open(crowdsale.url);
        b = await investor.contribute(crowdsale.currency.tiers[0].supply+1);
        assert.equal(b, false, "Test FAILED.Investor can  contribute more than supply in tier");
        logger.warn("Test PASSED. Investor can NOT contribute more than supply in tier");

    });

    test.it('Investor can contribute minCap', async function() {
        //await investor.setMetaMaskAccount();
        ...
        assert.equal(b, true, "Test FAILED.Investor can NOT contribute minCap");
        logger.warn("Test PASSED. Investor can contribute minCap");

    });

    test.it('Investor can contribute less than minCap after first transaction', async function() {
        investor=user77_27F2;
        await investor.open(crowdsale.url);
        balance=await investor.getBalanceFromPage(crowdsale.url);
        ...
        assert.equal(b, true, "Test FAILED. Investor can NOT contribute less than minCap after first transaction");
        logger.warn("Test PASSED. Investor can contribute less than minCap after first transaction" );

    });
    test.it.skip('Owner can NOT distribute before  all tokens were sold', async function() {

    });
    test.it.skip('Owner can NOT finalize before  all tokens were sold & if crowdsale NOT ended ', async function() {

    });

    test.it('Investor can contribute maximum', async function() {
       // await investor.setMetaMaskAccount();
        investor=user77_27F2;
        await investor.open(crowdsale.url);
        balance=await investor.getBalanceFromPage(crowdsale.url);
