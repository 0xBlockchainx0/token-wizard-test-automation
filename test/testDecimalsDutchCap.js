...
test.describe('POA token-wizard. Test purchase with all possible values of DECIMALS, Dutch, whitelist', async function () {
	...
	const scenario = './scenarios/scenarioDutchDecimalsCap.json';
...
/////////////////////////////////////////////////////////////////////////
	...
	test.before(async function () {
		...
		console.log("Owner's balance = " + await Utils.getBalance(Owner) / 1e18 + " Eth");
	...
	for (let i=15;i<19;i++)
	{
		...
		test.it('Investor is able to buy amount equal minCap',
			async function () {
				...
				let contribution = crowdsale.tiers[0].minCap;
		...
		test.it('Investors balance is properly changed after purchase ',
			async function () {
				...
				console.log("ShouldBe = " + crowdsale.tiers[0].minCap);
				let result = (balanceTokens >= crowdsale.tiers[0].minCap);
